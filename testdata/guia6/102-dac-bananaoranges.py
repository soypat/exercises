# Divide and conquer
'''
★★★ *Divide y conquistarás*. Miguela vive en un pueblo frutero en el valle de Oz.
...
#### Entrada
Se espera una entrada ingresada por usuario de texto del estilo `"011000110101F"` donde
cada `0` representa una naranja y cada `1` representa una banana. `F` siempre
esta presente al final de cada entrada. Las primeras
frutas en llegar a la cinta transportadora son las de la izquierda, es decir los
primeros valores de la cadena. En el ejemplo `"0010F"` primero llegan dos naranjas
...
En el ejemplo `"1D01D0F"` entra primero una banana, como tiene la letra `D` que la
sigue esa banana es descartada. La tercer fruta en entrar también es una banana que
...
Definiciones:
I. FRUTA(S): Puede referirse a una banana o naranja no defectuosa, o un conjunto mezclado de ambas.
